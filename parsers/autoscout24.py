"""
Parser d'email d'alerte AutoScout24.
Format : emails "Nouveaux résultats correspondant à votre recherche"
liens vers /offers/{slug}-{id}
"""
...
ID_PATTERNS = [
    # AutoScout24 utilise des UUIDs pour les offres : /offers/{uuid}
    re.compile(r"autoscout24\.[a-z]+/offers/([0-9a-f-]{32,40})", re.IGNORECASE),
    # Ancien format avec ID numérique
    re.compile(r"autoscout24\.[a-z]+/[^?]*?[-/](\d{9,})(?:[?#]|$)", re.IGNORECASE),
]
...
def parse(raw_email) -> List[Listing]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if "autoscout24" not in href.lower():
            continue
        ...
        listings.append(Listing(
            ...
            title=title or "Annonce AutoScout24",
            price=price,
            year=year,
            mileage=mileage,
            location=location,
            url=href if href.startswith("http") else f"https://www.autoscout24.fr/offers/{listing_id}",
            image_url=image_url,
...
def _extract_title(a_tag: Tag, block: Optional[Tag]) -> Optional[str]:
    ...
    if a_text and len(a_text) >= 10 and not a_text.lower().startswith(("voir", "consulter", "http", "details")):
        return a_text[:200]
    if block:
...
def _extract_location(text: str) -> Optional[str]:
    m = re.search(r"([A-ZÀ-Ÿ][A-Za-zÀ-ÿ\-' ]{2,30})\s*(?:\(|-|\s)(\d{2,5})", text)
    if m:
    ...
    for pat in ID_PATTERNS:
        for m in pat.finditer(text):
            ...
            listings.append(Listing(
                site="autoscout24",
                listing_id=listing_id,
                title="Annonce AutoScout24",
                price=extract_price(text),
                year=extract_year(text),
                mileage=extract_mileage(text),
                location=None,
                url=f"https://www.autoscout24.fr/offers/{listing_id}",
            ))
    return listings
